...
export function OwnerMarketingPanel() {
  ...
  async function saveHeaderColors() {
    if (!config) return;
    setLoading(true);
    const headerColors = mergeHeaderColors(config.headerColors);
    const res = await ownerMarketingFetch("/api/superadmin/config", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ headerColors }),
      credentials: "include",
    });
    const data = (await res.json()) as { config?: SiteOwnerSingleton; error?: string };
    setLoading(false);
  ...
  async function uploadVideo(form: HTMLFormElement) {
    ...
    try {
      ...
      const initRes = await ownerMarketingFetch("/api/superadmin/videos/upload-init", {
        method: "POST",
        headers: { "content-type": "application/json" },
        credentials: "include",
      ...
      const completeRes = await ownerMarketingFetch("/api/superadmin/videos/upload-complete", {
        method: "POST",
        headers: { "content-type": "application/json" },
        credentials: "include",
  ...
  async function deleteVideo(id: string) {
    const res = await ownerMarketingFetch(`/api/superadmin/videos/${encodeURIComponent(id)}`, {
      method: "DELETE",
      credentials: "include",
    });
    if (!res.ok) {
  ...
  if (!config) {
    return (
      <div className="mx-auto max-w-lg space-y-3 px-4 py-16">
        ...
        <p className="text-sm text-slate-600">{msg ?? "Could not load settings."}</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 px-4 py-6">
      ...
      {tab === "headercolors" ? (
        <HeaderColorsEditor
          config={mergeHeaderColors(config.headerColors)}
          onChange={(headerColors) => setConfig({ ...config, headerColors })}
          onSave={() => void saveHeaderColors()}
          saving={loading}
        />
      ) : null}
...
function SpecialsPopupImageControls({
  ...
  async function onUpload(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await ownerMarketingFetch("/api/superadmin/specials-popup-image/upload", {
        method: "POST",
        body: fd,
        credentials: "include",
      });
  ...
  async function onRemove() {
    ...
    setBusy(true);
    try {
      const res = await ownerMarketingFetch(`/api/superadmin/specials-popup-image?variant=${encodeURIComponent(variant)}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = (await res.json().catch(() => ({}))) as { error?: string };
...
function DoctorMediaTab({
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    onMessage(null);
    try {
      const initRes = await ownerMarketingFetch("/api/superadmin/doctor-media/upload-init", {
        method: "POST",
        headers: { "content-type": "application/json" },
        credentials: "include",
      ...
      const completeRes = await ownerMarketingFetch("/api/superadmin/doctor-media/upload-complete", {
        method: "POST",
        headers: { "content-type": "application/json" },
        credentials: "include",
  ...
  async function move(id: string, dir: -1 | 1) {
    ...
    await ownerMarketingFetch("/api/superadmin/doctor-media/reorder", {
      method: "POST",
      headers: { "content-type": "application/json" },
          ...
          .map((m) => (
            <li key={m.id} className="flex flex-wrap items-center gap-2 border-t border-slate-100 py-2">
              ...
              <form
                ...
                onSubmit={async (e) => {
                  e.preventDefault();
                  await ownerMarketingFetch(`/api/superadmin/doctor-media/${encodeURIComponent(m.id)}`, {
                    method: "DELETE",
                    credentials: "include",
                  });
                  await onReload();
